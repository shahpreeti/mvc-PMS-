...
<%@page import="com.javatpoint.SubFormBean"%>
<%@page import="com.javatpoint.AppraiseBean"%>
<%@page import="com.javatpoint.SaveSubFormBean"%>
...
<title>Sub Ordinates Form</title>
</head>
<body>
<%SubFormBean subformbean=(SubFormBean)request.getAttribute("subformbean"); 
...
int i=0,j=0,k=0;
String status=savesubformbean.getAppraiseStatus();
...
%>
	...
	<%
		String[][] sectionform=subformbean.getForm(j);
		int slen=sectionform.length;
		%><label>displaying section<%=j+1 %></label>
		<button id="filledBySub" onclick="displayAppraisee()">Appraisee' form</button><br><%
		if(sectionform[0][9].equals("Y"))
		{
			...
			<%for(int t=0;t<sectionform.length;t++)
			{
				...
				elementdisable = sectionform[t][8].equals("1")?"readonly":"";
				btndisable = sectionform[t][8].equals("1")?"disabled":"";
				%>
				<tr>
